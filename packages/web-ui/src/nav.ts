...
/** App path without leading slash, e.g. "dashboard" or "agencies/new". */
export function routeFromPathname(pathname: string): string {
  const clean = pathname.replace(/^\/+/, "").replace(/\/+$/, "");
  return clean || "dashboard";
}

/** Normalize any legacy hash or relative link to an absolute app path. */
export function toAppPath(to: string): string {
...
export function portalNav(portal: Portal): NavItem[] {
  if (portal === "platform") {
    return [
      ...
      { href: "/permissions", label: "Permissions", path: "/api/v1/platform/permissions" },
      { href: "/security", label: "Security", path: "/api/v1/auth/mfa/methods" },
