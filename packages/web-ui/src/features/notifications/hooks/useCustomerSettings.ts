...
export function useCustomerSettings() {
    ...
    try {
      const [prefRows, inboxRows] = await Promise.all([
        asList<PreferenceRow>(
          await apiGet<unknown>("/api/v1/customer/notification-preferences"),
        ),
        apiGet<unknown>("/api/v1/customer/notifications")
  ...
  function patchPreference(
    eventType: string,
    patch: Partial<Pick<PreferenceRow, "email" | "in_app">>,
  ) {
    setPreferences((rows) =>
  ...
  async function savePreferences() {
    ...
    try {
      const saved = asList<PreferenceRow>(
        await apiSend<unknown>("/api/v1/customer/notification-preferences", "PUT", {
  ...
  async function markRead(notificationId: string) {
    setBusy(true);
    try {
      await apiSend(`/api/v1/customer/notifications/${notificationId}/read`, "POST", {});
      setInbox((rows) =>
        rows.map((row) =>
