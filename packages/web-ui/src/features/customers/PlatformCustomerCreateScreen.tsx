...
import { useCreatePlatformCustomer } from "@/features/customers/hooks/useCreatePlatformCustomer";
...
export function PlatformCustomerCreateScreen() {
  const navigate = useNavigate();
  const { createCustomer, agencies, loadingAgencies, error, busy, setError } =
  ...
  function validateStep(current: StepId): string {
    if (current === "account") {
      if (!form.agency_id) return "Agency is required.";
        ...
        {step === "account" ? (
          <div className="grid gap-4">
            <FormSelect
              ...
              onChange={(event) => update("agency_id", event.target.value)}
