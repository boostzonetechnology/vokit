...
import { useAgencyCustomerList } from "@/features/customers/hooks/useAgencyCustomerList";
import { CUSTOMER_STATUSES } from "@/features/customers/types";

export function AgencyCustomersScreen() {
  ...
  const { customers, error, loading } = useAgencyCustomerList({
    status: statusFilter,
    ...
    <section className="mx-auto max-w-[1200px]">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 mb-0 text-body text-text-muted">
            Customers under your agency · AG2
          </p>
        </div>
      ...
      <article className="rounded-xl border border-border-default bg-surface p-5 shadow-subtle">
        ...
        {loading && customers.length === 0 ? (
          <TableSkeleton
            headers={["Customer", "Owner", "Status", "Created"]}
            rows={8}
          />
        ) : customers.length === 0 ? (
            ...
            <table className="min-w-full">
              <thead>
                <tr className="text-label uppercase text-text-muted">
                  ...
                  <th className="border-0 px-2 py-2 text-left">Owner</th>
              ...
              <tbody>
                {customers.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-3 text-text-secondary">{row.owner_email || "—"}</td>
                    <td className="px-2 py-3">
